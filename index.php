<?php
	...
	$Channel_Secret = "7b03f71e4c835ee13089d7f190c8e932bad";
						...
						if(strcasecmp($messageType, "text") == 0){
							...
							if(strlen($messageText) > 0){
								if(strcmp(substr($messageText, 0, 1), "/") == 0){ //Bot Command
									...
									}else if(strcasecmp(substr($messageText, 1), "cekpassword") == 0){
									ReplyMessage($linereplyurl, $Channel_Access_Token, $replyToken, array(NewText("*Silahkan ketik password atau kata kunci yang ingin kalian cek tingkat keamanannya :")));
										if($userId){
											$passw = $messageText;
											if( preg_match( '/[a-z]+/', $passw ) && 
											  preg_match( '/[A-Z]+/', $passw ) && 
											  preg_match( '/[0-9]+/', $passw ) && 
											  preg_match( '/[[:punct:]]+/', $passw ) && sizeof($passw) >10 )
											{
												$reactionmsg = NewText("Password kamu udah kuat nih! karena udah cukup panjang dan memiliki kombinasi yang kuat!");
												ReplyMessage($linereplyurl, $Channel_Access_Token, $replyToken, $reactionmsg);
											}
											else if (sizeof($passw) >6 && preg_match( '/[a-z]+/', $passw ) && 
											  preg_match( '/[A-Z]+/', $passw ) || preg_match( '/[0-9]+/', $passw ) && preg_match( '/[[:punct:]]+/', $passw ))
											{
												$reactionmsg = NewText("Password kamu udah belum cukup kuat nih! karena kurang memiliki kombinasi karakter yang kuat, kalian boleh cek password lainnya atau gunakan password yang aku generate aja. hehe \n\nKetik: \"/cekpassword\" untuk cek password kamu yang lain");
												ReplyMessage($linereplyurl, $Channel_Access_Token, $replyToken, $reactionmsg);
											}
											else
											{
												$reactionmsg = NewText("Password kamu masih lemah atau rawan banget nih! karena kurang memiliki kombinasi karakter yang kuat, kalian boleh cek password lainnya atau gunakan password yang aku generate aja. hehe \n\nKetik: \"/cekpassword\" untuk cek password kamu yang lain");
												ReplyMessage($linereplyurl, $Channel_Access_Token, $replyToken, $reactionmsg);
											}
										}
									}
